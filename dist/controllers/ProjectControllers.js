...
const ProjectFunctions_1 = require("../helper/ProjectFunctions");
...
const createProjectByName = async (req, res) => {
    try {
        const { name, description } = req.body;
        const isExist = await ProjectModel_1.default.findOne({ name });
        if (isExist) {
            return res.status(400).json({
                success: false,
                error: req.i18n.t("projectValidationMessages.response.createProjectByName.exist"),
            });
        }
        ...
        return res.status(201).json({
            success: true,
            message: req.i18n.t("projectValidationMessages.response.createProjectByName.success"),
            project: newProject,
        });
    }
    catch (error) {
        return res.status(500).json({
            success: false,
            error: req.i18n.t("projectValidationMessages.response.createProjectByName.server"),
        });
    }
};
exports.createProjectByName = createProjectByName;
...
const createProject = async (req, res) => {
    ...
    try {
        ...
        await session.withTransaction(async (session) => {
            ...
            const { parentProjectId, name, description, country, region, worksite } = req.body;
            let parentProject;
            // Validation checks without manual transaction handling
            if (!parentProjectId && !name) {
                throw new Error("Please Provide atleast one one of them Parent Project or New Project Name.");
            }
            if (!parentProjectId && !description) {
                throw new Error("Please Provide atleast one one of them Parent Project or New Project Description.");
            }
            if (parentProjectId) {
                parentProject = await ProjectModel_1.default.findById(parentProjectId).session(session);
                if (!parentProject) {
                    throw new Error(`${req.i18n.t("projectValidationMessages.response.createProject.parentProject")} ${parentProjectId}.`);
                }
            }
            ...
            if (!isCountryExist || !isRegionExist || !isWorksiteExist) {
                throw new Error(`${!isCountryExist
                    ? `Country: ${country}`
                    : !isRegionExist
                        ? `Region: ${region}`
                        : `Worksite ${worksite}`} ${req.i18n.t("projectValidationMessages.response.createProject.invalidLocation")}`);
            }
            let projectLocation = await LocationModel_1.default.findOne({
                country,
                region,
                worksite,
            }).session(session);
            if (!projectLocation) {
                [projectLocation] = await LocationModel_1.default.create([
                    {
                        country,
                        region,
                        worksite,
            ...
            const id = await (0, ProjectFunctions_1.generateUniqueId)();
            const projectData = {
                id,
            ...
            const [newProject] = await ProjectModel_1.default.create([projectData], {
                session,
            });
            let ownerRole = await RoleModel_1.default.findOne({
                title: "Owner",
            }).session(session);
            if (!ownerRole) {
                [ownerRole] = await RoleModel_1.default.create([
                    {
                        title: "Owner",
            ...
            await ProjectRoleModel_1.default.create([projectRoleData], { session });
            const populatedProject = await ProjectModel_1.default.findById(newProject._id)
                .populate({
                path: "location",
        ...
        return res.status(201).json({
            success: true,
            message: req.i18n.t("projectValidationMessages.response.createProject.success"),
...
const getAllProjects = async (req, res) => {
    ...
    try {
        ...
        if (role === global_enum_1.GlobalAdminRoles.SuperAdmin) {
            let options = (0, pagination_1.getPaginationOptions)(req, {
                populate: projectPopulateOptions,
                sort: { createdAt: -1 },
                filter: {
                    createdBy: new mongoose_1.default.Types.ObjectId(currentUser.id),
                },
            });
            if (search) {
                options = (0, pagination_1.getPaginationOptions)(req, {
                    populate: projectPopulateOptions,
                    sort: { createdAt: -1 },
                    filter: {
                        createdBy: new mongoose_1.default.Types.ObjectId(currentUser.id),
            ...
            result = await (0, pagination_1.paginate)(ProjectModel_1.default, options);
        }
        else if (role === global_enum_1.GlobalAdminRoles.ClientAdmin) {
            const employee = await EmployeeModel_1.default.findOne({
                user: new mongoose_1.default.Types.ObjectId(currentUser.id),
            });
...
const getProjectById = async (req, res) => {
    ...
    try {
        ...
        const roles = await ProjectRoleModel_1.default.aggregate(rolesPipeline);
        // Construct response
        const data = {
            id: project.id,
            name: project.name,
...
const updateProject = async (req, res) => {
    ...
    try {
        if (parentProjectId) {
            const isParentProjectExist = await ProjectModel_1.default.findById(parentProjectId);
            if (!isParentProjectExist) {
                throw new Error(`${req.i18n.t("projectValidationMessages.response.updateProject.parentProject")} ${parentProjectId}.`);
            }
        }
        if (location) {
            const isLocationExist = await LocationModel_1.default.exists({ _id: location });
            if (!isLocationExist) {
                return res.status(200).json({
                    success: false,
                    error: `${req.i18n.t("projectValidationMessages.response.updateProject.locationNotExist")} ${location}.`,
                });
            }
        }
        const updatedProject = await ProjectModel_1.default.findByIdAndUpdate(id, { name, description, location, parentProjectId }, { new: true, runValidators: true });
        if (!updatedProject) {
            return res.status(200).json({
                success: false,
                message: req.i18n.t("projectValidationMessages.response.updateProject.notFound"),
            });
        }
        return res.status(200).json({
            success: true,
            message: req.i18n.t("projectValidationMessages.response.updateProject.success"),
...
const deleteProject = async (req, res) => {
    const { id } = req.params;
    try {
        const project = await ProjectModel_1.default.findByIdAndUpdate(id, {
            $set: {
                isDeleted: true,
            },
        });
        if (!project) {
            return res.status(200).json({
                success: false,
                message: req.i18n.t("projectValidationMessages.response.deleteProjectById.notFound"),
            });
        }
        return res.status(200).json({
            success: true,
            message: req.i18n.t("projectValidationMessages.response.deleteProjectById.success"),
        });
    }
    catch (error) {
        return res.status(500).json({
            success: false,
            error: req.i18n.t("projectValidationMessages.response.deleteProjectById.server"),
        });
    }
};
...
//   }
// };
const getProjectsByLocation = async (req, res) => {
    try {
        const { country, region, worksite } = req.query;
        const location = await LocationModel_1.default.findOne({
            country,
            region,
            worksite,
        });
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(location._id)) {
            return res.status(400).json({
                success: false,
        ...
        const projects = await ProjectModel_1.default.find({
            location: new mongoose_1.default.Types.ObjectId(location._id),
            isDeleted: false,
        }).populate({
            path: "location",
            select: "country region worksite",
        ...
        return res.status(500).json({
            success: false,
            error: req.i18n.t("projectValidationMessages.response.getProjectsByLocation.server"),
        });
    }
};
exports.getProjectsByLocation = getProjectsByLocation;
const getAllEmployeesInProjectOrganization = async (req, res) => {
    try {
        const { id } = req.params;
...
exports.getAllEmployeesInProjectOrganization = getAllEmployeesInProjectOrganization;
