...
import  ConversationModel, {ConversationType, IConversation, IParticipant } from '../models/ConversationModel';
...
class ConversationService {
   ...
   */
  async createConversation(friendlyName: string, createdBy: string): Promise<IConversation> {
    try {
   ...
   */
  async addParticipant(conversationId: string, userId: string, identity: string): Promise<IConversation> {
    try {
      ...
      const participant = await conversationsClient.conversations(conversation.twilioSid)
        .participants
        .create({ identity });
  ...
  async removeParticipant(conversationId: string, userId: string): Promise<IConversation> {
    try {
      ...
      await conversationsClient.conversations(conversation.twilioSid)
   ...
   */
  async sendMessage(conversationId: string, userId: string, body: string, media?: string[]): Promise<any> {
    try {
      const conversation = await ConversationModel.findById(conversationId);
      if (!conversation) {
        throw new Error('Conversation not found');
      ...
      const twilioMessage = await conversationsClient.conversations(conversation.twilioSid)
        .messages
        .create({ 
          body,
   ...
   */
  async getConversationMessages(conversationId: string, userId: string, limit = 50, before?: string): Promise<any> {
    try {
      const conversation = await ConversationModel.findById(conversationId);
      if (!conversation) {
        throw new Error('Conversation not found');
   ...
   */
  async updateConversation(conversationId: string, userId: string, updates: { friendlyName?: string, attributes?: any }): Promise<IConversation> {
    try {
      const conversation = await ConversationModel.findById(conversationId);
      if (!conversation) {
        throw new Error('Conversation not found');
      ...
      if (updates.attributes) updateParams.attributes = JSON.stringify(updates.attributes);
  ...
  async deleteConversation(conversationId: string, userId: string): Promise<boolean> {
    try {
      ...
      await MessageModel.deleteMany({ conversationSid: conversation.twilioSid });
