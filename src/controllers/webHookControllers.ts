...
import ConversationModel, { IParticipant } from '../models/ConversationModel';
...
const handleMessageAdded = async (
  ...
  try {
    ...
    const existingMessage = await MessageModel.findOne({ messageSid });
    if (existingMessage) {
    ...
    const conversation = await ConversationModel.findOne({ twilioSid: conversationSid });
    if (!conversation) {
    ...
    if (!userId) {
      const participant = conversation.participants.find((p:any) => p.identity === author);
      if (participant) {
 ...
 */
const handleConversationRemoved = async (conversationSid: string) => {
  try {
    // Delete the conversation and all messages
    await ConversationModel.deleteOne({ twilioSid: conversationSid });
    await MessageModel.deleteMany({ conversationSid });
 ...
 */
const handleParticipantAdded = async (data: any) => {
  try {
 ...
 */
const handleParticipantRemoved = async (data: any) => {
  try {
